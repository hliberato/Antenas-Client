<template>
  <form
    class="login-form"
    @submit="proceed($event)"
  >
    <h5 class="login-form__title">
      {{ getTitleMessage() }}
    </h5>
    ...
    <div
      v-if="role == 'REPRESENTATIVE' && signUp"
      class="login-form__field"
    >
      <v-text-field
        v-model="city"
    ...
    proceed (event) {
      ...
      if (this.signUp) {
        ...
        UserService
          ...
          .catch(() => {
            this.notificationType = 'error-register'
        ...
        this.$store.dispatch('authenticateUser', {
          ...
          .catch(() => {
            this.notificationType = 'error-login'
            this.notificationVisible = true
          })
          .finally(() => loader.hide())
      }
